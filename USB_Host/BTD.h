...
#define IOGEAR_GBU421_VID       0x0A5C // The IOGEAR GBU521 dongle does not presents itself correctly, so we have to check for it manually
#define IOGEAR_GBU421_PID       0x2148
...
public:
        ...
        virtual boolean VIDPIDOK(uint16_t vid, uint16_t pid) {
                if(vid == IOGEAR_GBU521_VID && pid == IOGEAR_GBU521_PID)
                        return true;
                if(vid == IOGEAR_GBU421_VID && pid == IOGEAR_GBU421_PID)
                                        return true;
