...
uint8_t BTD::ConfigureDevice(uint8_t parent, uint8_t port, bool lowspeed) {
        ...
        p->lowspeed = lowspeed;
#if defined(GRSAKURA)
        //We need to discover the size of the default endpoint.
        rcode = pUsb->getDevDescr(0, 0, p->epinfo->maxPktSize, buf);
        //We now have the max packet size with which we can set up the Sakura's FIFO depth.
        p->epinfo->maxPktSize = udd->bMaxPacketSize0;
#endif
...
uint8_t BTD::Init(uint8_t parent, uint8_t port, bool lowspeed) {
        ...
        if(VID == PS3_VID && (PID == PS3_PID || PID == PS3NAVIGATION_PID || PID == PS3MOVE_PID)) {
                ...
                for(uint8_t i = 0; i < num_of_conf; i++) {
                        ...
                        }else if(VID == IOGEAR_GBU421_VID && PID == IOGEAR_GBU421_PID) {
                            ConfigDescParser<USB_CLASS_VENDOR_SPECIFIC, WI_SUBCLASS_RF, WI_PROTOCOL_BT, CP_MASK_COMPARE_ALL> confDescrParser(this); // Needed for the IOGEAR GBU521
                            rcode = pUsb->getConfDescr(bAddress, 0, i, &confDescrParser);
                        }else {
...
#if defined(GRSAKURA)
        epInfo[index].epAttribs = (pep->bmAttributes);
#endif
#ifdef EXTRADEBUG
...
#if defined(TESTING)
      // Now we can initialize bAddress to 0;
        bAddress = 0;   // Clear device address (search term)
#endif
        return 0;
}
